...
window.addEventListener('load', function() {
  var droplet = document.getElementById('myRectangle');
  var width = window.innerWidth;
  var height = window.innerHeight;
  var x = Math.floor(Math.random() * (width - droplet.offsetWidth));
  var y = Math.floor(Math.random() * (height - droplet.offsetHeight));
  droplet.style.left = x + 'px';
  droplet.style.top = y + 'px';
});


// викликаємо Hammer з елементом draggable
var droplet = document.querySelector('.draggable');
var hammertime = new Hammer(droplet);

// визначаємо можливість переміщення об'єкта
var isDragging = false;
var currentPosX = 0;
var currentPosY = 0;

// визначаємо подію переміщення об'єкта
...
hammertime.on('panmove', function(ev) {
  if (isDragging) {
    var posX = ev.deltaX + currentPosX;
    var posY = ev.deltaY + currentPosY;

    droplet.style.transform = 'translate(' + posX + 'px, ' + posY + 'px)';
  }
});

hammertime.on('panend', function(ev) {
  isDragging = false;
  currentPosX += ev.deltaX;
  currentPosY += ev.deltaY;
});

function closeDetailsMenu() {
    var detailsElement = document.getElementById("Details-menu-drawer-container");
    detailsElement.removeAttribute("open");
  }